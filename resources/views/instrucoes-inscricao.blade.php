<!DOCTYPE html>
<html lang="en">
    @include('includes.head')

<body>


  <div class="sticky-top">
    @include('includes.navbar')
  </div>

  <div class="container">
    <main>
      <h4 class="display-3"> Instruções para inscrição</h4>
      <div class="mt-4">
        <p>
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Alias voluptatibus nam cumque eveniet, voluptates provident iure, aperiam doloribus itaque aliquam illo dignissimos? Voluptatem minima unde in enim laudantium, nisi accusamus!
          Ipsum blanditiis id illo quam corrupti maxime! Ut quod temporibus ab vel quasi ducimus, quibusdam ea repellat sint ullam! Officia amet modi tempora accusantium eveniet temporibus unde! Odit, ab suscipit.
          Blanditiis commodi voluptate nemo animi placeat dolorem ex repellat maxime. Quod facilis libero autem? Harum architecto quas commodi, accusamus magnam quibusdam magni, eligendi, omnis culpa exercitationem saepe maxime rem error.
          Dolore voluptate deserunt quo excepturi consectetur? Nostrum sit ratione nihil beatae atque rerum dolor facere. Rerum odit culpa, optio quam tempora temporibus dolorem error corrupti. Nostrum obcaecati ipsum minima repellat!
          Commodi maxime, consectetur eligendi, facere consequatur non hic eum recusandae tempora beatae aliquam eveniet ducimus ab necessitatibus corporis quos ipsam ut quibusdam praesentium quae molestiae impedit voluptas, totam reprehenderit. Pariatur?Lorem ipsum dolor sit amet consectetur adipisicing elit. Alias voluptatibus nam cumque eveniet, voluptates provident iure, aperiam doloribus itaque aliquam illo dignissimos? Voluptatem minima unde in enim laudantium, nisi accusamus!
          Ipsum blanditiis id illo quam corrupti maxime! Ut quod temporibus ab vel quasi ducimus, quibusdam ea repellat sint ullam! Officia amet modi tempora accusantium eveniet temporibus unde! Odit, ab suscipit.
          Blanditiis commodi voluptate nemo animi placeat dolorem ex repellat maxime. Quod facilis libero autem? Harum architecto quas commodi, accusamus magnam quibusdam magni, eligendi, omnis culpa exercitationem saepe maxime rem error.
          Dolore voluptate deserunt quo excepturi consectetur? Nostrum sit ratione nihil beatae atque rerum dolor facere. Rerum odit culpa, optio quam tempora temporibus dolorem error corrupti. Nostrum obcaecati ipsum minima repellat!
          Commodi maxime, consectetur eligendi, facere consequatur non hic eum recusandae tempora beatae aliquam eveniet ducimus ab necessitatibus corporis quos ipsam ut quibusdam praesentium quae molestiae impedit voluptas, totam reprehenderit. Pariatur?
        </p>
        <div class="d-block">
          <p class="font-weight-bold d-inline">Critérios para inscrições de trabalhos:</p> <a target="_blank" href="./link-criterios-trabalho.php"> clique aqui.</a>
          <!-- add JS para abrir nova pagina pop-up -->
        </div>
        <p class="font-weight-bold d-inline"> Dúvidas? </p> <a href="mailto:email@email.com.br?subject=Dúvida">Mande uma mensagem.</a>
      </div>
      <div class="title mt-4">
        <h5> Valores <h5>
      </div>
      <div class="table">
        <table class="table mt-4">
          <thead>
            <tr>
              <th scope="col">Categoria</th>
              <th scope="col">Faixa 1</th>
              <th scope="col">Faixa 2</th>
              <th scope="col">Local</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">Estudante Associado</th>
              <td>R$ 00,00</td>
              <td>R$ 00,00</td>
              <td>R$ 00,00</td>
            </tr>
            <tr>
              <th scope="row">Professor Associado</th>
              <td>R$ 00,00</td>
              <td>R$ 00,00</td>
              <td>R$ 00,00</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="title mt-4">
        <h5> Sobre os valores <h5>
      </div>
      <div>
        <p> Lorem ipsum dolor sit amet consectetur adipisicing elit. Aspernatur ducimus blanditiis, omnis quaerat placeat quae voluptas similique ullam consequatur laboriosam harum, amet est debitis nesciunt assumenda architecto consequuntur sequi sed.
          Similique sint quis modi eum omnis magni. Vero praesentium aperiam eius ea, rem facere consectetur enim eaque dignissimos exercitationem cum error magnam qui repudiandae iure veniam aut, ad mollitia? Architecto?
          Veniam, accusantium consectetur nam, porro modi est praesentium doloribus quo possimus ipsam quis obcaecati sapiente numquam illum sequi esse? Doloremque vitae, omnis aliquid veritatis nobis animi totam optio accusamus vero?
          Dolorum architecto, maxime nobis rem possimus officia aliquam voluptate. A accusantium iusto adipisci, corrupti cumque ullam aperiam excepturi fuga pariatur nulla et at provident ipsa earum. Est, expedita. Aspernatur, minus.
          Rem, architecto ratione magnam saepe dolore adipisci, cum quia iusto sequi laboriosam, ducimus unde repudiandae sunt omnis sed debitis. Magnam culpa quisquam voluptates, quam nulla maiores quas rem. Rerum, voluptatem!
          Natus omnis sapiente, iure ex doloribus architecto id deserunt quisquam eius suscipit debitis harum tenetur. Iusto sequi soluta et labore dignissimos, cum, tempora doloremque beatae, nemo quidem sint explicabo ad.
        </p>
      </div>
      <div class="title mt-4">
        <h5> Políticas de reembolso <h5>
      </div>
      <div>
        <p> Lorem ipsum dolor sit amet consectetur adipisicing elit. Aspernatur ducimus blanditiis, omnis quaerat placeat quae voluptas similique ullam consequatur laboriosam harum, amet est debitis nesciunt assumenda architecto consequuntur sequi sed.
          Similique sint quis modi eum omnis magni. Vero praesentium aperiam eius ea, rem facere consectetur enim eaque dignissimos exercitationem cum error magnam qui repudiandae iure veniam aut, ad mollitia? Architecto?
          Veniam, accusantium consectetur nam, porro modi est praesentium doloribus quo possimus ipsam quis obcaecati sapiente numquam illum sequi esse? Doloremque vitae, omnis aliquid veritatis nobis animi totam optio accusamus vero?
          Dolorum architecto, maxime nobis rem possimus officia aliquam voluptate. A accusantium iusto adipisci, corrupti cumque ullam aperiam excepturi fuga pariatur nulla et at provident ipsa earum. Est, expedita. Aspernatur, minus.
          Rem, architecto ratione magnam saepe dolore adipisci, cum quia iusto sequi laboriosam, ducimus unde repudiandae sunt omnis sed debitis. Magnam culpa quisquam voluptates, quam nulla maiores quas rem. Rerum, voluptatem!
          Natus omnis sapiente, iure ex doloribus architecto id deserunt quisquam eius suscipit debitis harum tenetur. Iusto sequi soluta et labore dignissimos, cum, tempora doloremque beatae, nemo quidem sint explicabo ad.
        </p>
        <a class="font-weight-bold" href="#"> Clique aqui para fazer o download da ficha de reembolso.</a>
      </div>
      <div class="mt-4 d-flex justify-content-center">
        <a href="./inscricao-usuario.php" class="p-1 px-5 mb-3 btn btn-outline-primary" role="button"> Clique aqui para realizar sua inscrição</a>
      </div>
    </main>
  </div>

  <div>
    @include('includes.footer')
    @include('includes.script')
  </div>

</body>

</html>